			...
			<InspectorControls>
				<PanelBody title={__('Layout', 'goodblocks')}>
					<SelectControl
						...
						value={layout}
						options={[
							{ label: 'Vertikal (bild ovanför)', value: 'vertical' },
							{ label: 'Horisontell (bild vänster)', value: 'horizontal' },
							{ label: 'Overlay (text över bild)', value: 'overlay' },
						]}
						...
						<MediaUpload
							...
							render={({ open }) => (
								<div>
									{mediaUrl ? (
										<>
											<img src={mediaUrl} alt="" style={{ width: '100%', marginBottom: '8px' }} />
											<Button isSecondary onClick={open} style={{ marginRight: '8px' }}>
												{__('Byt bild', 'goodblocks')}
											</Button>
											<Button isDestructive onClick={() => setAttributes({ mediaUrl: '', mediaId: undefined })}>
...
const Save = ({ attributes }) => {
	...
	const wrapperProps = linkUrl
		? { href: linkUrl, target: linkTarget, rel: linkTarget === '_blank' ? 'noopener noreferrer' : undefined }
		: {};
