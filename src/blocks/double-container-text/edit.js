...
const Edit = ( { attributes, setAttributes } ) => {
	...
	return (
		<div { ...useBlockProps() }>
			<InspectorControls>
				<PanelBody
					title={ __( 'Left Container Settings', 'goodblocks' ) }
					initialOpen={ true }
				>
					<TextControl
					...
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( { leftPosition: value } )
						}
					/>
					<SelectControl
					...
					<MediaUploadCheck>
						<MediaUpload
							onSelect={ ( media ) =>
								setAttributes( { leftMedia: media?.id || 0 } )
							}
				...
				<PanelBody
					title={ __( 'Right Container Settings', 'goodblocks' ) }
					initialOpen={ false }
				>
					<TextControl
					...
					<SelectControl
						...
						options={ positionOptions }
						onChange={ ( value ) =>
							setAttributes( { rightPosition: value } )
						}
					/>
					<SelectControl
						label={ __( 'Text Color', 'goodblocks' ) }
						...
						options={ colorOptions }
						onChange={ ( value ) =>
							setAttributes( { rightColor: value } )
						}
					/>
					<RangeControl
						label={ __( 'Overlay opacity', 'goodblocks' ) }
					...
					<MediaUploadCheck>
						<MediaUpload
							onSelect={ ( media ) =>
								setAttributes( { rightMedia: media?.id || 0 } )
							}
							allowedTypes={ [ 'image', 'video' ] }
							value={ rightMedia }
							render={ ( { open } ) => (
