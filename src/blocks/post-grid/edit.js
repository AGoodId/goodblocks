...
const Edit = ( { attributes, setAttributes } ) => {
	...
	let activePostTypes = [ 'post' ];
	if ( selectedPostTypes && selectedPostTypes.length > 0 ) {
		activePostTypes = selectedPostTypes;
	} else if ( attributes.postType ) {
		activePostTypes = [ attributes.postType ];
	}
	...
	const ssrAttributes = {
		...
		taxonomyTerms: Array.isArray( taxonomyTerms )
			? taxonomyTerms.join( ',' )
			: taxonomyTerms || '',
	};
			...
			<InspectorControls>
				<PanelBody title={ __( 'Inställningar', 'goodblocks' ) }>
					...
					<RangeControl
						...
						max={ 100 }
						help={ __(
							'Använd -1 för att visa alla inlägg',
							'goodblocks'
						) }
					/>
					<RangeControl
						...
						onChange={ ( value ) =>
							setAttributes( { postsPerRow: value } )
						}
						min={ 1 }
						max={ 4 }
					/>
				...
				<PanelBody
					title={ __( 'Innehållskällor', 'goodblocks' ) }
					initialOpen={ true }
				>
					...
					{ allPostTypes &&
						allPostTypes.map( ( type ) => (
							<CheckboxControl
								key={ type.slug }
								label={
									type.labels?.singular_name || type.name
								}
								checked={ activePostTypes.includes(
									type.slug
								) }
								onChange={ ( checked ) =>
									handlePostTypeToggle( type.slug, checked )
								}
							/>
						) ) }
					...
					<SelectControl
						...
						options={ [
							{
								label: __( 'Välj taxonomi', 'goodblocks' ),
								value: '',
							},
				...
				<PanelBody
					title={ __( 'Sortering & filter', 'goodblocks' ) }
					initialOpen={ false }
				>
					<SelectControl
					...
					{ showMoreLink && (
						<>
							...
							<TextControl
								...
								value={ moreLinkUrl }
								onChange={ ( value ) =>
									setAttributes( { moreLinkUrl: value } )
								}
