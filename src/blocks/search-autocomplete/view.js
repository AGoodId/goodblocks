...
class SearchAutocompleteBlock {
	...
	cacheElements() {
		this.trigger = this.container.querySelector('.search-autocomplete__trigger');
		this.form = this.container.querySelector('.search-autocomplete__form');
		this.input = this.container.querySelector('.search-autocomplete__input');
		this.results = this.container.querySelector('.search-autocomplete__results');
		this.clearBtn = this.container.querySelector('.search-autocomplete__clear');
		this.closeBtn = this.container.querySelector('.search-autocomplete__close');
		this.backdrop = this.container.querySelector('.search-autocomplete__backdrop');
	}
	...
	bindEvents() {
		...
		document.addEventListener('keydown', (event) => {
			...
			this.closeResults();
			if (this.settings.expandable && this.container.classList.contains('is-expanded')) {
				this.collapse();
			}
		});
	}
	...
	setupExpandable() {
		...
		if (this.closeBtn) {
			this.closeBtn.addEventListener('click', this.collapse.bind(this));
		}

		if (this.backdrop) {
			this.backdrop.addEventListener('click', this.collapse.bind(this));
		}
	}
	...
	handleKeydown(event) {
		const items = this.results.querySelectorAll('.search-autocomplete__result');
		...
		switch (event.key) {
			...
			case 'Enter':
				if (this.selectedIndex >= 0 && items[this.selectedIndex]) {
					event.preventDefault();
					const link = items[this.selectedIndex].querySelector('.search-autocomplete__result-link');
					if (link) {
	...
	async loadSuggestions(query = '') {
		...
		const cacheKey = `__suggestions_${this.settings.suggestionsType}_${this.settings.maxResults}_${normalizedQuery.toLowerCase()}`;
		if (this.cache.has(cacheKey)) {
			this.renderSuggestions(this.cache.get(cacheKey), normalizedQuery);
			return;
		}
		...
		try {
			...
			const response = await fetch(`${this.settings.suggestionsUrl}?${params}`);
			if (!response.ok) {
	...
	async search(query) {
		...
		try {
			...
			const response = await fetch(`${this.settings.apiUrl}?${params}`, {
	...
	renderSuggestions(results, query = '') {
		...
		this.results.innerHTML = results
			.map((item, index) => this.renderResultItem(item, index, query))
			.join('');
	...
	renderResults(results, query) {
		...
		if (!Array.isArray(results) || results.length === 0) {
			const noResultsText = (searchI18n.noResults || 'No results for "%s"').replace('%s', this.escapeHtml(query));
	...
	renderResultItem(item, index, query = '') {
		const thumbnail = this.settings.showThumbnail && item.thumbnail
			? `<img src="${this.escapeAttribute(item.thumbnail)}" alt="" class="search-autocomplete__thumb" loading="lazy" />`
			: '';

		const type = this.settings.showType && item.type
			? `<span class="search-autocomplete__type">${this.escapeHtml(item.type)}</span>`
			: '';

		const title = this.highlightText(item.title || '', query);
		const excerpt = this.settings.showExcerpt && item.excerpt
			? `<p class="search-autocomplete__excerpt">${this.highlightText(item.excerpt, query)}</p>`
			: '';
		...
		return `
			<div class="search-autocomplete__result" role="option" data-index="${index}">
				<a href="${this.escapeAttribute(item.url || '#')}" class="search-autocomplete__result-link">
					${thumbnail}
	...
	renderTermPills(terms) {
		...
		const items = terms
			...
			.map((term) => {
				...
				const taxonomyLabel = this.escapeAttribute(term.taxonomy_label || term.taxonomy || 'Kategori');
	...
	highlightText(text, query) {
		...
		return parts
			.map((part, index) => {
				if (index % 2 === 1) {
					return `<mark class="search-autocomplete__highlight">${this.escapeHtml(part)}</mark>`;
				}
	...
	renderViewAllLink(query) {
		const viewAllUrl = `${window.location.origin}/?s=${encodeURIComponent(query)}`;
		const viewAllText = (searchI18n.viewAll || 'View all results for "%s"').replace('%s', this.escapeHtml(query));
		return `
			<a href="${this.escapeAttribute(viewAllUrl)}" class="search-autocomplete__view-all">
				${viewAllText}
